...
def make_dataloaders(train_dataset=None, val_dataset=None, test_dataset=None, 
    ...
    FLOWERS_MEAN = np.array([0.485, 0.456, 0.406]) * 255
    FLOWERS_STD = np.array([0.229, 0.224, 0.225]) * 255
    loaders = {}
...
def construct_model(arch):
    if arch == 'resnet':
        model = models.resnet18(num_classes=196)
        #model = models.resnet50(pretrained=True)
        #for param in model.parameters():
        #    param.requires_grad = False
        
        #model.fc = Linear(in_features=model.fc.in_features, out_features=196)
        #for param in model.fc.parameters():
        #    param.requires_grad = True
    
    elif arch == 'mobilenet':
        model = models.mobilenet_v2(num_classes = 196)
    elif arch == 'efficientnet':
        model = models.efficientnet_b0(num_classes = 196)
    elif arch == 'regnet':
        model = models.regnet_x_400mf(num_classes = 196)
